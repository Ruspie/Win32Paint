...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	AppendMenu(MainMenu, MF_STRING, ID_COLOR_LINE, L"Pen color");
	AppendMenu(MainMenu, MF_STRING, ID_COLOR_FILL, L"Fill color");
	AppendMenu(MainMenu, MF_STRING, ID_NO_COLOR_FILL, L"No fill");
			...
			switch (LOWORD(wParam))
			{
				...
				case ID_SAVE_AS_FILE:
				{
					...
					if (GetSaveFileName(&ofn) == TRUE)
					{
						...
						//StringCchCat(ofn.lpstrFile, sizeof(ofn.lpstrFile) + 5, L".emf");
